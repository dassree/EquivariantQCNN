...
def data_load_and_process(dataset, classes=[0, 1], feature_reduction="resize1024", binary=True):
    if dataset == 'fashion_mnist':
        ...
        x_test = []
        y_test = []
        f1 = open('patchCamelyon/train_data_x.pkl', 'rb')
        f2 = open('patchCamelyon/train_data_y.pkl', 'rb')
        f3 = open('patchCamelyon/test_data_x.pkl', 'rb')
        f4 = open('patchCamelyon/test_data_y.pkl', 'rb')
#       
        x_train = np.array(pickle.load(f1))
        y_train = np.array(pickle.load(f2))
        x_test = np.array(pickle.load(f3))
        y_test = np.array(pickle.load(f4))
    ...
    if feature_reduction == 'resize256':
        if dataset=="cifar10" or dataset == "bloodmnist":
            ...
            pad1 = np.full((1000, 32, 32, 1), 0)
            pad2 = np.full((200, 32, 32, 1), 0)
            X_train = np.append(X_train, pad1, axis=3)
            X_test = np.append(X_test, pad2, axis=3)
            X1_train = tf.reshape(X_train[:], (np.shape(X_train)[0], 4096, 1, 1))
            X1_test = tf.reshape(X_test[:], (np.shape(X_test)[0], 4096, 1, 1))
