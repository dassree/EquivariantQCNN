...
def Benchmarking(dataset, classes, Unitaries, U_num_params, Encodings, circuit, cost_fn, binary=True):
    ...
    for j in range(J):
        ...
        print("\n")
        print("Loss History for " + circuit + " circuits, " + str(Unitaries) + " " + Encoding + " with " + cost_fn)
        loss_history, trained_params = Training.circuit_training(X_train, Y_train, Unitaries, U_num_params, Embedding, circuit, cost_fn)

        if circuit == 'QCNN':
            predictions = np.array([QCNN_circuit.QCNN(x, trained_params, Unitaries, U_num_params, Embedding, cost_fn) for x in X_test])
            ...
            predictions = [Hierarchical_circuit.Hierarchical_classifier(x, trained_params, Unitaries, U_num_params, Embedding, cost_fn) for x in X_test]

        accuracy = accuracy_test(predictions, Y_test, cost_fn, binary)
        print("Accuracy for " + str(Unitaries) + " " + Encoding + " :" + str(accuracy))

        f.write("Loss History for " + circuit + " circuits, " + str(Unitaries) + " " + Encoding + " with " + cost_fn)
        f.write("\n")
        f.write(str(loss_history))
        f.write("\n")
        f.write("Accuracy for " + str(Unitaries) + " " + Encoding + " :" + str(accuracy))
        f.write("\n")
        f.write("\n")
    f.close()
