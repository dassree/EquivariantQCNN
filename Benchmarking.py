...
def Benchmarking(dataset, classes, Unitaries, U_num_params, Encoding, circuit, cost_fn, processNumber, binary=True):
    ...
    np.random.seed(processNumber)
    f = open(f'patchCamelyon/patch-{str(processNumber)}_eqv.txt', 'w')
    ...
    learning_rate = 0.01
    batch_size = 20
    total_params = U_params[0] + U_params[1] + 6
    ...
    for it in range(steps):
        ...
        if it % 10 == 0:
            print("Proc " + str(processNumber) + " iteration: " + str(it+1) + "params: " + str(params) + " cost: ", str(cost_new))
